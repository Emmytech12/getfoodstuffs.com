
<footer class="footer-div">  
    <div class="footer-div-in"> 
        <div class="segment-back-div">
            <div class="segment-div" data-aos="fade-up" data-aos-duration="900">
                <div class="logo-div"><img src="<?php echo $website_url?>/all-images/images/logo.png" alt="<?php $thename?> Logo"></div>
                <p>As a Nigerian student, juggling school, assignments, and daily life can be overwhelming. That’s why Get Food Stuffs is here—to make your life easier by delivering fresh, affordable raw food straight to your hostel, dorm, or apartment.</p>

                <div class="icon-div">
                    <button class="social-icon" title="Facebook"><i class="bi-facebook"></i></button>
                    <button class="social-icon" title="Whatsapp"><i class="bi-whatsapp"></i></button>
                    <button class="social-icon" title="Instagram"><i class="bi-instagram"></i></button>
                    <button class="social-icon" title="LinkedIn"><i class="bi-linkedin"></i></button>
                    <button class="social-icon" title="Twitter"><i class="bi-twitter-x"></i></button>
                </div>
            </div>

            <div class="segment-div" data-aos="fade-up" data-aos-duration="1000">
                <h3>Quick Link</h3>
                <ul>
                    <a href="<?php echo $website_url?>" title="Sign Up/Login">
                    <li><i class="bi-arrow-right-short"></i>Sign Up/Login</li></a>
                    <a href="<?php echo $website_url?>/about" title="About Us">
                    <li><i class="bi-arrow-right-short"></i> About Us</li></a>
                    <a href="<?php echo $website_url?>/contact" title="Contact Us">
                    <li><i class="bi-arrow-right-short"></i> Contact Us</li></a>
                    <a href="<?php echo $website_url?>/blog/" title="Blog & Articles">
                    <li><i class="bi-arrow-right-short"></i> Blog & Articles</li></a>
                    <a href="<?php echo $website_url?>/faq" title="Frequently Asked Questions">
                    <li><i class="bi-arrow-right-short"></i> Frequently Asked Questions</li></a>  
                    <a href="<?php echo $website_url?>" title="Terms And Conditions">
                    <li><i class="bi-arrow-right-short"></i> Terms And Conditions</li></a> 
                    <a href="<?php echo $website_url?>" title="Privacy Policy">
                    <li><i class="bi-arrow-right-short"></i> Privacy Policy</li></a>                
                </ul>
            </div>

            <div class="segment-div services" data-aos="fade-up" data-aos-duration="1200">
                <h3>Product Categories</h3>
                <ul>
                <a href="<?php echo $website_url?>" title="Froozen Foods">
                    <li><i class="bi-arrow-right-short"></i> Froozen Foods</li></a> 
                    <a href="<?php echo $website_url?>" title="Bulk/Wholesales Market">
                    <li><i class="bi-arrow-right-short"></i>Bulk/Wholesales Market</li></a>
                    <a href="<?php echo $website_url?>" title="Drink & Beverages">
                    <li><i class="bi-arrow-right-short"></i> Drink & Beverages</li></a>                                                             
                    <a href="<?php echo $website_url?>" title="Fresh Meat & Livestock">
                    <li><i class="bi-arrow-right-short"></i> Fresh Meat & Livestock</li></a>  
                    <a href="<?php echo $website_url?>" title="Pasta & Packages Foods">
                    <li><i class="bi-arrow-right-short"></i> Pasta & Packages Foods</li></a> 
                    <a href="<?php echo $website_url?>" title="Seasonings & Spices">
                    <li><i class="bi-arrow-right-short"></i> Seasonings & Spices</li></a> 
                    <a href="<?php echo $website_url?>" title="Veggies">
                    <li><i class="bi-arrow-right-short"></i> Veggies</li></a> 
                </ul>
            </div>

            <div class="segment-div" data-aos="fade-up" data-aos-duration="1400">
                <h3>Quick Contact</h3>
                <div class="contact-back-div">
                    <div class="contact-div">
                        <div class="icon-div"><i class="bi-telephone-inbound"></i></div>
                        <div class="text-div"><span> (+234) 812 700 0262</span></div>
                    </div>

                    <div class="contact-div">
                        <div class="icon-div"><i class="bi-geo-alt-fill"></i></div>
                        <div class="text-div"><span>Lagos, Nigeria.</span></div>
                    </div>

                    <div class="contact-div">
                        <div class="icon-div"><i class="bi-envelope-fill"></i></div>
                        <div class="text-div"><span>info@getfoodstuffs.com</span></div>
                    </div>
                </div>

                <h3>Newsletter</h3>

                <div class="newsletter-text-div">
                    <div class="footer_text_field_container">
                        <input class="footer_text_field" type="email" id="email" placeholder="" />
                        <div class="placeholder">Enter Your Email</div>
                    </div>
                    <button class="btn" title="Send Mail" onclick=""><i class="bi-send"></i></button>
                </div>
            </div> 
            <br clear="all" />
        </div>
    </div>
    
    <div class="main-bottom-div">
        <div class="div-in">
            <p>&copy; 2022 - <?php echo date("Y") ?> <span><?php echo $thename?></span>. All Rights Reserved.</p>
            <p>Developed By: <a href="https://webliststudio.com/" target="_blank"><span class="text"> Weblist Studio Solution Inc</span></a></p>
        </div>
    </div>
</footer>

<?php include 'bottom-scripts.php'?>